        ...
        <div className="pb-24">
                ...
                <div className="grid grid-cols-1 gap-x-10 gap-y-4 md:grid-cols-2">
                    <TF l="My Company">
                        <Select value={form.company_id} onValueChange={(v) => set("company_id", v)} disabled={isView}>
                            <SelectTrigger data-testid="s-company"><SelectValue placeholder="Please Select" /></SelectTrigger>
                            <SelectContent>
                    ...
                    <TF l={<Req>ID Type</Req>}>
                        <Select value={form.id_type} onValueChange={(v) => set("id_type", v)} disabled={isView}>
                            <SelectTrigger data-testid="s-id-type"
                    ...
                    <TF l={<Req>ID Value</Req>}>
                        <Input value={form.brn} onChange={(e) => set("brn", e.target.value)}
                               ...
                               className="bg-yellow-50 dark:bg-yellow-500/10" data-testid="s-id-value" />
                    </TF>
                    <TF l={<Req>TIN</Req>}>
                        <Input value={form.tin} onChange={(e) => set("tin", e.target.value)}
                               placeholder="e.g. C24700902040" disabled={isView}
                               className="bg-yellow-50 dark:bg-yellow-500/10" data-testid="s-tin" />
                    </TF>

                    <TF l={<Req>Name</Req>}>
                        <Input value={form.name} onChange={(e) => set("name", e.target.value)}
                               placeholder="e.g. Acme Sdn Bhd" disabled={isView}
                               className="bg-yellow-50 dark:bg-yellow-500/10" data-testid="s-name" />
                    </TF>
                    <TF l="SST Registration Number">
                        <Input value={form.sst_registration_number}
                               ...
                               placeholder="NA" disabled={isView} data-testid="s-sst" />
                    </TF>

                    <TF l={<Req>Contact Number</Req>}>
                        <Input value={form.phone} onChange={(e) => set("phone", e.target.value)}
                               placeholder="e.g. +60312345678" disabled={isView}
                               className="bg-yellow-50 dark:bg-yellow-500/10" data-testid="s-contact" />
                    </TF>
                    <TF l="E-mail">
                        <Input type="email" value={form.email} onChange={(e) => set("email", e.target.value)}
                               placeholder="e.g. billing@acme.my" disabled={isView} data-testid="s-email" />
                    </TF>

                    <TF l="Malaysia Standard Industrial Classification">
                        <CodeSelect options={MSIC_CODES} value={form.msic_code}
                    ...
                    <TF l="Authorisation Number For Certified Exporter" full>
                    ...
                    <TF l="Business Activity Description" full>
                        <Textarea rows={2} value={form.business_activity}
                                  onChange={(e) => set("business_activity", e.target.value)} disabled={isView}
                                  data-testid="s-activity" />
                    </TF>
                </div>
                    ...
                    <div className="grid grid-cols-1 gap-x-10 gap-y-4 md:grid-cols-2">
                        <TF l={<Req>Country</Req>}>
                            <Select value={form.country} onValueChange={(v) => set("country", v)} disabled={isView}>
                                <SelectTrigger data-testid="s-country"
                        ...
                        <TF l={<Req>State</Req>}>
                            <Select value={form.state} onValueChange={(v) => { set("state", v); set("city", ""); }}
                                ...
                                <SelectTrigger data-testid="s-state"
                                    className="bg-yellow-50 dark:bg-yellow-500/10"><SelectValue placeholder="Please Select" /></SelectTrigger>
                                <SelectContent>
                        ...
                        <TF l={<Req>City Name</Req>}>
                            <Select value={form.city} onValueChange={(v) => set("city", v)}
                                ...
                                <SelectTrigger data-testid="s-city"
                                    className="bg-yellow-50 dark:bg-yellow-500/10"><SelectValue placeholder="Please Select" /></SelectTrigger>
                                <SelectContent>
                        ...
                        <TF l={<Req>Address Line 0</Req>}>
                            <Input value={form.addr_line_0} onChange={(e) => set("addr_line_0", e.target.value)}
                                   ...
                                   className="bg-yellow-50 dark:bg-yellow-500/10" data-testid="s-addr-0" />
                        </TF>
                        ...
                        <TF l="Postal Zone">
                            <Input value={form.postal_zone} onChange={(e) => set("postal_zone", e.target.value)}
                                   placeholder="e.g. 50450" disabled={isView} data-testid="s-postal" />
                        </TF>
                    </div>
                </div>
            </div>
            ...
            <div className="fixed inset-x-0 bottom-0 z-30 border-t border-border bg-primary py-3">
                <div className="flex justify-center gap-3">
                    {!isView && (
                        <Button variant="secondary" onClick={submit} data-testid="s-submit">
                            <Check className="mr-2 h-4 w-4" /> Submit
                        </Button>
                    )}
                    <Button variant="secondary" onClick={() => nav("/suppliers")} data-testid="s-cancel">
