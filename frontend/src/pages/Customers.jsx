...
export default function Customers() {
    ...
    const doDelete = async () => {
        ...
        if (!window.confirm(`Delete ${selected.size} buyer(s)?`)) return;
        try {
            for (const id of selected) await api.delete(`/customers/${id}`);
    ...
    const doToggle = async (enable) => {
        ...
        try {
            for (const id of selected) {
                ...
                if (!row) continue;
                const { id: _, ...rest } = row;
                await api.put(`/customers/${id}`, { ...rest, enabled: enable });
            }
    ...
    const doExport = () => {
        const header = ["NO.", "My Company", "ID Type", "ID Value", "TIN", "Name",
                        ...
                        "Buyer Code", "Status", "Operation Date"];
        const csv = [header.join(",")].concat(
            rows.map((r, i) => [
                i + 1, q(companyName(r.company_id, companies) || ""),
                q(r.id_type || "Business Registration Number"),
                q(r.brn || ""), q(r.tin || ""), q(r.name || ""),
                q(r.sst_registration_number || ""), q(r.phone || ""), q(r.email || ""),
                q(r.buyer_code || ""),
        ...
        a.href = url; a.download = `buyers_${Date.now()}.csv`; a.click();
    ...
    return (
        <div className="pb-16">
            <PageHeader kicker="Master data" title="Buyers"
                subtitle="LHDN-approved buyer registry — Section C parties used across e-invoices." />
            ...
            <div className="mb-3 flex flex-wrap items-center gap-2">
                <Button asChild variant="outline" size="sm" data-testid="op-add">
                    <Link to="/customers/new"><Plus className="mr-2 h-3.5 w-3.5" /> Add</Link>
                </Button>
                <Button variant="outline" size="sm" data-testid="op-modify"
                        disabled={selected.size !== 1}
                        onClick={() => { const id = oneSelected(); if (id) nav(`/customers/${id}/edit`); }}>
                ...
                <Button variant="outline" size="sm" data-testid="op-view"
                        disabled={selected.size !== 1}
                        onClick={() => { const id = oneSelected(); if (id) nav(`/customers/${id}/edit?view=1`); }}>
                ...
                <BatchImportButton entity="customers"
                ...
                <div className="max-h-[70vh] overflow-auto rounded-md border border-border bg-card">
                    <table className="w-full min-w-[1500px] text-sm">
                        ...
                        <tbody>
                            ...
                            {rows.length === 0 && (
                                <tr><td colSpan={13} className="py-10 text-center text-sm text-muted-foreground">No Data</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
